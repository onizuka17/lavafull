              ...
              <!-- form start -->
                ...
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <textarea  class="textarea form-control" :class="{'is-invalid':form.errors.has('description')}" v-model="form.description" name="description" placeholder="Nhập nội dung"></textarea>
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control" name="parent_id" v-model="form.parent_id" :class="{ 'is-invalid': form.errors.has('parent_id') }">
                       ...
                       <option v-for="item in listcate" v-if="checkValue(item)" :value="item.id" >{{item.title}}</option>
                    </select>
  ...
  function string_to_slug(str) {
    ...
    computed : {
      ...mapState({
		...
		methods : {
      checkValue(item){
          if (item.id == this.$route.params.catid || item.parent_id == this.$route.params.catid) {
            return false;
          }else{
            return true;
          }
			...
			addCate(){
				this.form.put('/api/admin/update-category/'+this.$route.params.catid)
				 	.then((res) => { 
    ...
    created(){
      ...
      var form_fill=this.form;
       axios.get('/api/admin/get-category/'+this.$route.params.catid)
      .then(function (response) {  
        //var cate={
            //title : response.data.category.title
        //};
        //console.log(cate);
        form_fill.fill(response.data.category);
      });
